...
exports.NetManager = NetManager_1 = class NetManager {
    ...
    callAroundClientFun(player, fun, ...params) {
        ...
        Events__default["default"].DispatchToAllClient(player, this.NOTIFY, funName, ...params);
    }
    /**
     * 调用所有客户端的方法
     * @param fun 方法路径|方法
...
class Effect {
    ...
    playHandle() {
        ...
        if (this.effectData.angles != null) {
            this.go.SetRelativeRotation(new Type__default["default"].Rotation(this.effectData.angles));
        }
        else {
            this.go.SetRelativeRotation(Type__default["default"].Rotation.ZERO);
        }
        if (this.effectData.scale != null) {
            this.go.scale = this.effectData.scale;
        }
        else {
            this.go.scale = Type__default["default"].Vector.ONE;
        }
    }
    /**编辑器特效对象 */
    get mwEffect() {
...
exports.EffectManager = EffectManager_1 = class EffectManager {
        ...
        return new Promise((resolve) => {
            ...
            let id = setInterval(() => {
                for (; waitIndex < effDataArr.length; waitIndex++) {
                    ...
                    if (guid.length > 10 && MWCore__default["default"].GameObject.Find(guid) == null) {
                        break; //等不到就一直等
                    }
                    else {
     ...
     */
    playEffectInPlayer(resId, player, socketType, loopNum = 1, offset = Type__default["default"].Vector.ZERO, angles = null, scale = null) {
     ...
     * @returns playId，本次播放的唯一标识，可用于停止
     */
    playEffectInGameObject(resId, target, loopNum = 1, offset = Type__default["default"].Vector.ZERO, angles = null, scale = null) {
     ...
     */
    getEffect(playId) {
        if (GamePlay__default["default"].IsServer() || !this.playingEffectMap.has(playId))
            return null;
...
class Sound {
    ...
    constructor(resId) {
        if (resId != null) {
            let go = MWCore__default["default"].GameObject.SpawnGameObject(resId); //同步
            if (go == null) {
    ...
    play3D(loopNum, volume) {
        ...
        this.go.SetSoundSphere(1000, 1, false, GamePlay__default["default"].MWAttenuationDistanceModel.Linear);
        this.volume = volume;
        this.go.PlayVoice();
        this._isDone = false;
    }
...
exports.InputManager = InputManager_1 = class InputManager {
    ...
    touchBegin() {
        ...
        let list = GamePlay__default["default"].GetClickGameobjectByScene(pos.x, pos.y, 50000, true, false);
        //this.log(list);
        let arr = [];
...
//挂载在ui文件上的UIpanel
class PanelBase extends MWGameUI__default["default"].MWUIBehaviour {
    uiPrefab = null;
    _visible = true;
    _eventListener;
    isEnabel = false;
    ...
    static creat(prefabPath) {
        ...
        let uiPrefab = MWGameUI__default["default"].CreateUIPrefabByName(prefabPath); //这个加载是同步的，prefab上的脚本也会同步创建（直接构造）
        if (uiPrefab != null) {
...
class UI extends PanelBase {
    ...
    static getScreenSize() {
        ...
        return new Type__default["default"].Vector2(x.value, y.value);
    }
    static getPlayerController() {
        return GamePlay__default["default"].GetCurrentPlayer().Character.Actor.GetPlayerController();
    }
    // 获取缩放比例
    static getCanvasScale() {
        let canvasSize = UI.getCanvasSize();
        let screenSize = UI.getScreenSize();
        return new Type__default["default"].Vector2(canvasSize.x / screenSize.x, canvasSize.y / screenSize.y);
    }
    /**
     * 获取屏幕尺寸
     * @returns 屏幕尺寸
    ...
    static getCanvasPointByWorld(worldPos) {
        ...
        p.value.X *= scale.x;
        p.value.Y *= scale.y;
        return new Type__default["default"].Vector2(p.value.X, p.value.Y);
    }
}
...
class AnalyticsUtil {
    ...
    static init() {
        ...
        this.msgMap = new Map();
        if (GamePlay__default["default"].IsClient()) {
            Events__default["default"].AddServerListener(AnalyticsUtil.NET_MSG_SEND_MGS, (eventName, eventDesc, jsonData) => {
    ...
    send(player) {
        ...
        if (GamePlay__default["default"].IsClient()) {
            ...
            if (player == null) {
                Events__default["default"].DispatchToAllRoomClient(AnalyticsUtil.NET_MSG_SEND_MGS, eventName, eventDesc, jsonData);
            }
            else {
                Events__default["default"].DispatchToClient(player, AnalyticsUtil.NET_MSG_SEND_MGS, eventName, eventDesc, jsonData);
            }
        }
    }
}
...
class OdinGame extends MWCore__default["default"].MWScript {
    ...
    OnStart() {
        if (GamePlay__default["default"].IsClient()) {
            ...
            let id = setInterval(() => {
                if (UI.instance != null) {
                    ...
                    id = setInterval(() => {
                        ...
                        Events__default["default"].DispatchToServer(this.CONNECT_SERVER);
                    }, 1000);
                    let listener = Events__default["default"].AddServerListener(this.CONNECT_SERVER, () => {
                        clearInterval(id);
                        listener.Disconnect();
                        this.enterGame(); //进入游戏
                    });
            ...
            this.enterGame(); //进入游戏
            Events__default["default"].AddClientListener(this.CONNECT_SERVER, (player) => {
                Events__default["default"].DispatchToClient(player, this.CONNECT_SERVER);
            });
        }
    }
    OnUpdate(dt) {
    ...
    async initClient() {
        ...
        exports.LogManager.instance.logWarning("____________________    MyPlayerID:" + GamePlay__default["default"].GetCurrentPlayer().GetPlayerID() + "   ____________________");
        let loadResult = true;
...
__decorate([
    MWCore__default["default"].MWProperty({ displayName: "Log级别" }) //0-3 0:不输出 3:输出所有
...
class NetBehaviour extends Behaviour {
    ...
    callWorldClientFun(funName, ...params) {
        if (funName == null || GamePlay__default["default"].IsClient())
            return;
        exports.NetManager.instance.callWorldClientFun(`${this.targetGuid}.${funName}`, ...params);
    }
}

class NetBehaviourCreator extends MWCore__default["default"].MWScript {
    _behaviour;
...
class SuperPanelBase {
    ...
    static creat(prefabPath) {
        ...
        panel.uiPrefab = MWGameUI__default["default"].CreateUIPrefabByName(path);
        if (panel.uiPrefab != null) {
            panel.uiPrefab.GetRootContent().SetVisibility(MWGameUI__default["default"].ESlateVisibility.SelfHitTestInvisible);
            oTrace('    Load Panel Success!');
        }
        else {
            oTraceError('    Load Panel Load Fail');
    ...
    set visible(value) {
        this._visible = value;
        if (value) {
            this.uiPrefab.SetVisibility(MWGameUI__default["default"].ESlateVisibility.SelfHitTestInvisible);
        }
        else {
            this.uiPrefab.SetVisibility(MWGameUI__default["default"].ESlateVisibility.Collapsed);
        }
    }
    //可见性
    get visible() { return this._visible; }
...
class ViewBase {
    ...
    constructor(prefabPath) {
        this.uiPrefab = MWGameUI__default["default"].CreateUIPrefabByName(prefabPath);
    }
    findChildByPath(ObjClass, path) {
        let child = this.uiPrefab.FindChildByPath(path);
